#!/usr/bin/env bash
# wren installer.
#   curl -fsSL https://digimata.github.io/wren/install.sh | sh
#
...
#
# Apple Silicon only: releases ship a single arm64 build to match the
# daemon's MLX/Metal targets.
...
if [ "$(uname -s)" != "Darwin" ]; then
    red "wren is macOS-only (detected $(uname -s))"
    exit 1
fi

ARCH=$(uname -m)
if [ "$ARCH" != "arm64" ]; then
    red "wren requires Apple Silicon (detected $ARCH)"
    red "releases only ship arm64 builds."
    exit 1
fi
...
green "✓ wren ${TAG} installed at ${INSTALL_DIR}/${BIN_NAME}"
echo
echo "next:"
echo "  wren status          # check the TTS daemon"
echo "  wren say \"hello\"     # speak through it"
